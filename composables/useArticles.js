...
export const useArticles = () => {
  // 共用狀態：儀表板與內頁共享同一份 article 資料，避免各自重打 Supabase。
  const tableState = useSharedTableState('article')
  const articles = tableState.items
  ...
  const loadArticles = () => {
    ...
    if (!client) return Promise.resolve(articles.value)
