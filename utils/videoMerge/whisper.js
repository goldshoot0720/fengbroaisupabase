/**
 * Speech-to-text (Whisper via Transformers.js) for video-merge subtitles.
 * Ported from https://github.com/huang1988pioneer/VideoMerge
 * Decodes local audio via Web Audio API (blob URL alone is unreliable for Whisper).
 */
import { chunksToSrt, chunksToVtt } from './subtitle.js'
...
/** Manual slice length — short enough that WASM Whisper finishes in reasonable time */
const SLICE_SEC = 15
...
export async function ensureTranscriber(onStatus, onProgress, opts = {}) {
  if (transcriber && loadedModelId) {
    onStatus?.(`使用記憶體內模型（${loadedModelId}，無需重新下載）`)
    onProgress?.(1)
    return transcriber
  }
  ...
  loadPromise = (async () => {
    onStatus?.(
      preferred
        ? '載入語音辨識模型（優先讀取瀏覽器快取，通常較快）…'
        : '載入語音辨識模型（首次需下載，之後會快取）…'
    )
    ...
    const progress_callback = (p) => {
      if (p?.status === 'progress' && typeof p.progress === 'number') {
        ...
        onStatus?.(
          sawNetworkProgress
            ? `下載模型：${name} ${Math.round(p.progress)}%（首次）`
...
export async function decodeAudioForWhisper(file, onLog) {
  ...
  try {
    ...
    if (rms < 1e-4) {
      throw new Error('音訊幾乎無聲（或解碼失敗）。請確認 MP3 有人聲、音量正常。')
    }
  ...
  try {
    ...
    if (!(audio instanceof Float32Array) || !audio.length) {
      throw new Error('read_audio 回傳無效')
    }
 ...
 * @returns {{ timestamp: [number, number], text: string }[]}
 */
function parseAsrResult(result, audioDurationSec) {
  ...
  const pushChunk = (text, start, end) => {
    ...
    if (!t) return
    if (
      /^(\.|\.{2,3}|…|\?|\[Music\]|\[音樂\]|\[music\]|thank you\.?|thanks for watching\.?)$/i.test(
        t
      )
    ) {
      return
...
function withTimeout(promise, ms, label) {
  return new Promise((resolve, reject) => {
    const t = setTimeout(() => {
      reject(
        new Error(
          `${label} 逾時（>${Math.round(ms / 1000)} 秒）。瀏覽器內辨識較慢，請改用較短 MP3 或語音稿。`
        )
      )
    }, ms)
    promise.then(
...
async function transcribeInSlices(asr, waveform, baseKwargs, hooks) {
  ...
  const heartbeat = setInterval(() => {
    ...
    onStatus?.(`辨識進行中…已 ${elapsed} 秒（瀏覽器內執行較慢，請勿關閉分頁）`)
  }, 2000)
  ...
  try {
    for (let i = 0; i < numSlices; i++) {
      ...
      onLog?.(
        `ASR 切片 ${i + 1}/${numSlices}: ${startSec.toFixed(1)}–${endSec.toFixed(1)}s (${sliceDur.toFixed(1)}s, ${slice.length} samples)`
      )
      ...
      try {
        ...
        onLog?.(`切片 ${i + 1} 失敗：${err?.message || err}`)
        continue
      }
      ...
      onLog?.(
        `切片 ${i + 1} 完成：+${part.length} 句 · 累計 ${all.length} · 「${part.map((p) => p.text).join(' ').slice(0, 60)}」`
      )
...
export async function transcribeAudioToSubtitles(file, opts = {}) {
  ...
  // Prefer tiny for speed — base is often too slow in-browser
  ...
  onLog?.(
    `解析出 ${chunks.length} 句，文字：${fullText.slice(0, 120)}${fullText.length > 120 ? '…' : ''}`
  )

  if (!chunks.length) {
    throw new Error(
      '整段音訊辨識出空文字。請確認：① MP3 含清楚人聲 ② 音量足夠 ③ 非純音樂。純音樂通常無法產生字幕。若下載模型失敗，請改用較短 MP3 再試。'
    )
  }
...
export function getLoadedWhisperModelId() {
