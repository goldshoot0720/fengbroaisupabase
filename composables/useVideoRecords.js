...
export const useVideoRecords = () => {
  ...
  const loadVideos = async (options = {}) => {
    ...
    await loadCachedTable({
      ...
      fetcher: () => selectWholeTable(client, 'video', { order: 'created_at', ascending: false })
    })
  }
  ...
  const withCacheSync = (fn) => async (...args) => {
    ...
    if (!result || result.success !== false) rememberCachedTable('video', videos.value)
    return result
  }

  return { videos, loading, error, FIELDS, loadVideos, addVideo: withCacheSync(addVideo), updateVideo: withCacheSync(updateVideo), deleteVideo: withCacheSync(deleteVideo), importVideos: withCacheSync(importVideos) }
}
