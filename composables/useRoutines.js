...
export const useRoutines = () => {
  // 共用狀態：儀表板與內頁共享同一份 routine 資料，避免各自重打 Supabase。
  const tableState = useSharedTableState('routine')
  const routines = tableState.items
  ...
  const loadRoutines = () => {
    ...
    if (!client) return Promise.resolve(routines.value)
